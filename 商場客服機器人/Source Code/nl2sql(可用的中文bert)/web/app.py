...
from customer import get_task1_model, get_task2_model, get_val_seq, main_process
# import openai
# import pandas as pd
# import numpy as np
# import pickle
...
def init_global_vars():
    # 在這裡執行全域初始化
    app.config['task1_model'] = get_task1_model()    
    app.config['task2_model'],app.config['tokenizer'] = get_task2_model()
    app.config['val_seq'] = get_val_seq()
    print("init_global_vars")
...
def get_sql_by_question():
    sqls,cols = main_process(app.config['task1_model'], app.config['task2_model'],app.config['tokenizer'], app.config['val_seq'])
    return sqls,cols
...
def get_db_data():
    ...
    # 將 JSON 字串寫入檔案中
    with open('../TableQA/customer/data.json', 'w') as f:
        f.write(json_str)
    
    sqls, cols = get_sql_by_question()

    sql_qry = s2t(sqls[0])

    # sql_qry = s2t(get_sql_by_question_fm_openai(t2s(data['question'])))
    ...
    conn.close()
    return jsonify({'data': rows, 'cols':cols})
