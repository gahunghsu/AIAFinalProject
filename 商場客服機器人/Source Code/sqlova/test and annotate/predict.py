...
from sqlova.utils.utils_wikisql_evaluate import *
...
args.no_pretraining = True  # counterintuitive, but avoids loading unused models #terminal出現問題,先試試看改成False,還是不行
...
def load_wikisql_data_(path_wikisql, mode='train', toy_model=False, toy_size=10, no_hs_tok=False, aug=False):
    """ Load training sets
    """
    if aug:
        mode = f"aug.{mode}"
    ...
    #path_sql = os.path.join("./gagaQA", mode+'.jsonl')#path_wikisql, mode+'_tok.jsonl'
    #path_sql = os.path.join("./gagaQA", 'customer.jsonl')
    path_sql = os.path.join("./gaga_0516", 'customer.jsonl')
    
    if no_hs_tok:
        #path_table = os.path.join("./gagaQA", mode + '.jsonl')
        path_table = os.path.join("./gaga_0516", mode + '.jsonl')
    else:
        #path_table = os.path.join("./gagaQA", mode+'.jsonl')
        path_table = os.path.join("./gaga_0516", mode + '.jsonl')
    ...
    return data, table
dev_data, dev_table = load_wikisql_data_(args.data_path, mode="customer.tables", toy_model=args.toy_model, toy_size=args.toy_size, no_hs_tok=True) 
