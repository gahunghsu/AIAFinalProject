...
#https://pytorch.org/docs/stable/generated/torch.Tensor.requires_grad_.html#:~:text=requires_grad_%20%28%29%20%E2%80%99s%20main%20use%20case%20is%20to,autograd%20will%20begin%20to%20record%20operations%20on%20tensor.
...
#new
# Set the device ID of the GPU you want to use
device_id = 1

# Set the device to the selected GPU
device = torch.device(f'cuda:{device_id}' if torch.cuda.is_available() else 'cpu')

# Set the memory allocation for the selected GPU
torch.cuda.set_device(device)
torch.cuda.empty_cache()
torch.cuda.memory_allocated(device)    # Memory currently allocated

#new_allocation = 1024*1024*1024   # New memory allocation in bytes (e.g. 1GB)
torch.cuda.set_device(device)
torch.cuda.empty_cache()
torch.cuda.set_per_process_memory_fraction(0.5, device=device)   # Set allocation to 50%
torch.cuda.memory_allocated(device)    # Memory currently allocated after increasing allocation
...
def get_bert(BERT_PT_PATH, bert_type, do_lower_case, no_pretraining):
    bert_config_file = os.path.join(BERT_PT_PATH, f'bert_config_{bert_type}.json') #configuration file
    vocab_file = os.path.join(BERT_PT_PATH, f'vocab_{bert_type}.txt') #vocabulary file
    init_checkpoint = os.path.join(BERT_PT_PATH, f'pytorch_model_{bert_type}.bin')#pre-trained weights file
    ...
    #tokenizer.print_status() #在test gaga QA的時候要close
...
def get_models(args, BERT_PT_PATH, trained=False, path_model_bert=None, path_model=None):
    ...
    args.iS = bert_config.hidden_size * args.num_target_layers  # Seq-to-SQL input vector dimenstion 
                                       #The iS variable is then used throughout the code to specify the input                                         #size of    
                                        #various layers in the Seq-to-SQL model.
    ...
    if trained:
        assert path_model_bert != None  #If path_model_bert is None, it means that the required path for the model's 
                                        #pre-trained BERT file is not provided, which would prevent the model from 
...
"""
The code you provided loads a pre-trained BERT model from a specified file path path_model_bert. It first checks whether a GPU is available using torch.cuda.is_available(). If a GPU is available, it loads the model using torch.load() without specifying the map_location argument, which means it loads the model onto the GPU. If a GPU is not available, it loads the model using torch.load() with the map_location argument set to 'cpu', which means it loads the model onto the CPU. The loaded model's state dictionary is then loaded into the model_bert instance using its load_state_dict() method.

In PyTorch, a model's state dictionary refers to a Python dictionary object that maps each layer/parameter of the model to its corresponding tensor value. It contains all the learnable parameters of the model, such as weights and biases of the layers, and their associated gradients. The loaded model's state dictionary means the state dictionary of the model that is being loaded from a saved checkpoint file. It is the same as the state dictionary that was saved when the model was trained previously. By loading the state dictionary, we can restore the model to its previous trained state, including all the learned parameters.
...
def train(train_loader, train_table, model, model_bert, opt, bert_config, tokenizer,
          ...
          # dset_name=The name of the dataset being trained on (e.g., 'train', 'dev', 'test').
          # path_db =The path to the database used for evaluation
          # A data loader (in this case, train_loader) is an object in PyTorch that provides an iterable over a dataset, in mini-  #batches, during the training process. It loads the data into memory, applies any necessary transformations, and returns the data #in the form of mini-batches that can be fed to the model during training. The train_loader is used in this function to iterate #over the training dataset (train_table) in mini-batches, and feed the batches to the model for training.
          #The opt_bert argument in the train function is an optimizer for the BERT model, and if it is set to None, the BERT model #will not be fine-tuned during training. On the other hand, the --fine_tune flag in the command line specifies that the BERT model #should be fine-tuned during training.
#So if opt_bert is None and the --fine_tune flag is specified in the command line, the BERT model will not be fine-tuned because #the opt_bert argument takes precedence over the command line flag. However, if opt_bert is not None, the BERT model will be fine-#tuned according to the optimizer specified by opt_bert, regardless of whether the --fine_tune flag is specified in the command #line.
        
        
    model.train() #sets the models to training mode using their train() method.
    model_bert.train()
...
if __name__ == '__main__':
    ...
    if not args.trained:
        ...
        #path_model_bert = './data_and_model/model_bert_best.pt' #第一次跑200回的訓練時所用的路徑 #原來的 # 這邊沒有檔案,改
        #path_model = './data_and_model/model_best.pt'  #第一次跑200回的訓練時所用的路徑  #原來的 #這邊沒有檔案,改
        path_model_bert = './model_bert_best.pt'  #在停止terminal3的60幾回訓練之後,重新再做訓練,修改路徑  
        #用這個路徑發現跑evaluate_ws.py的良率更差,所以改回原來的 #改回這邊,調整run_train.sh的指令
        path_model = './model_best.pt' #在停止terminal3的60幾回訓練之後,重新再做訓練,修改路徑 
                                       # 用這個路徑發現跑evaluate_ws.py的良率更差,所以改回原來的
                                       #改回這邊,調整run_train.sh的指令
    ...
    if args.do_infer:
        ...
        # Set the path to the Stanford CoreNLP installation directory
        #corenlp_path = "xt121-group11/sqlova/stanford2/CoreNLP"

        # Set the environment variable
        os.environ["CORENLP_HOME"] = "./stanford2/CoreNLP"
        ...
        #client=corenlp.CoreNLPClient(annotators='ssplit,tokenize'.split(','),timeout= 30000)
        #client=CoreNLPClient(annotators='from stanza.server import CoreNLPClient'.split(','),output_format='text',timeout=                 30000, be_quiet=False)
        client=CoreNLPClient(annotators=['tokenize', 'ssplit', 'pos', 'lemma', 'ner', 'parse', 'depparse', 'coref', 'kbp', 'quote',         'entitymentions'],output_format='text',timeout=60000, be_quiet=False)
        ...
        #db_name = 'ctable'
        db_name = 'dev'
        #data_table = load_jsonl('./data_and_model/ctable.tables.jsonl') # ctable
